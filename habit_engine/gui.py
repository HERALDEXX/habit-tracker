...
from habit_engine.habit_io import DATA_DIR, get_asset_path, reset_app_data, clear_tracking_data, PLOTS_DIR
...
class HabitTrackerGUI:
    ...
    def _setup_autosave(self):
        ...
        def check_autosave():
            now = datetime.now()
            if self.pending_changes and (now - self.last_save).seconds >= 30:
                self._autosave()
    ...
    def setup_sidebar(self):
        ...
        # Clear logs button with custom styling
        self.clear_btn = self.ctk.CTkButton(
            self.sidebar,
        ...
        self.clear_btn.tooltip = None
        self.clear_btn.tooltip_timer = None
        self.clear_btn.grid(row=5, column=0, padx=20, pady=10)
        self.clear_btn.bind('<Enter>', lambda e: self._show_tooltip(e, self.clear_btn, "No Tracking History to Clear!"))
        self.clear_btn.bind('<Leave>', lambda e: self._hide_tooltip(e, self.clear_btn))    
        ...
        self.open_data_button = self.ctk.CTkButton(
            self.sidebar,
        ...
        self.open_data_button.grid(row=7, column=0, padx=20, pady=(5, 10))

        # Appearance mode selector with custom colors
        self.appearance_mode = self.ctk.CTkOptionMenu(
            self.sidebar,
        ...
        self.appearance_mode.grid(row=8, column=0, padx=20, pady=(10, 10))  # Adjusted padding

        # License button at bottom of sidebar
        license_btn = self.ctk.CTkButton(
            self.sidebar,
            text="View License",
        ...
        license_btn.grid(row=9, column=0, padx=20, pady=(0, 20))  # Added at bottom
    ...
    def open_plot_file(self, filepath, parent=None):
        try:
            ...
            self.set_window_icon(plot_window)
        ...
        try:
            ...
            def create_viz():
                try:
                    ...
                    result = self._visualize(self.logs, habit_name)
    ...
    def change_appearance_mode(self, new_mode: str):
        """Change the application's appearance mode (Light/Dark/System)"""
        try:
            # Store before changing in case of error
            old_mode = self.current_appearance_mode if (hasattr(self, 'current_appearance_mode')) else "system"
            ...
            if new_mode.lower() == "system":
                ...
                if system_theme:
                    self.ctk.set_appearance_mode(system_theme.lower())
                else:
                    self.ctk.set_appearance_mode("light")  # Fallback to light if detection fails
            else:
                self.ctk.set_appearance_mode(new_mode.lower())
                
            self.current_appearance_mode = new_mode
            
            # Update dropdown selection and persist the change
            if (hasattr(self, 'appearance_mode')):
                self.appearance_mode.set(new_mode)
            
            # Force all windows to update with new theme
            if (hasattr(self, 'window')):
                self.window.update_idletasks()
                
            # Show success message with a delay
            self.window.after(500, lambda: self.show_success_message(f"Theme changed to {new_mode} mode"))
                
        except Exception as e:
            # Revert on error
            try:
                self.ctk.set_appearance_mode(old_mode.lower())
                self.current_appearance_mode = old_mode
                if (hasattr(self, 'appearance_mode')):
                    self.appearance_mode.set(old_mode)
            except:
                pass
